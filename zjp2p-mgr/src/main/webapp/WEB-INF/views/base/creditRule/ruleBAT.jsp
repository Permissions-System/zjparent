        ...
        <form id="queryForm" method="post">
            <table class="datagrid-toolbar" style="width:100%;height:70%;border:0px;font-size:12px ">
                <tr class="itemMagin">
                    ...
                    <td><input id="actionSearch" class="easyui-validatebox" data-options="validType:['actionValid','length[0,20]']"/></td>
                    <td><label>规则名称:</label></td>
                    <td><input id="ruleNameSearch" name="ruleNameSearch" class="easyui-validatebox" data-options="validType:['nameValid','length[0,20]']"/></td>
                    ...
                    <td>
                        <select id="cycleTypeSearch" panelHeight="auto" name="cycleTypeSearch" class="easyui-combobox" style="width:155px;" data-option="required:false,editable:false">     
                            ...
                            <c:forEach items="${selects.CYCLE_TYPE_BAT}" var="type">
...
<table>
	...
	<tr>
		<td><label>动作:</label></td>
		<td><input class="easyui-validatebox" type="text" id="action" data-options="required:true,validType:['actionValid','length[4,20]']"/></td>
	</tr>
	<tr>
		<td><label>规则名称:</label></td>
		<td><input class="easyui-validatebox" type="text" id="ruleName" data-options="required:true,validType:['nameValid','length[4,20]']"/></td>
	</tr>
	<tr>
		<td><label>周期类型:</label></td>
		<td>
	...
	<tr>
		<td><label>周期最低重复次数:</label></td>
		<td><input class="easyui-validatebox" type="text" id="repeatNum" data-options="required:true,validType:['numberValid','length[0,8]']"/></td>
	</tr>
	<tr>
		<td><label>规则限制表达式:</label></td>
		<td><input class="easyui-validatebox" type="text" id="ruleLimit" title="此处设定规则限制表达式，表达式需要返回true/false，参数需以params开头，例：params.expense>233" class="easyui-tooltip" data-options="validType:['limitValid']"/></td>
	</tr>
	<tr>
		<td><label>积分计算表达式:</label></td>
		<td><input class="easyui-validatebox" type="text" id="credit" title="此处设定积分计算表达式，表达式需要返回一个整形数值，参数需以params为开头，例：(params.expense-100)/2" class="easyui-tooltip" data-options="required:true,validType:'creditValid'"/></td>
	</tr>
	<tr>
...
$('#creditRuleList').datagrid({    
	url:'${ctx}/base/creditRule/bat/list',  
	title:'批量规则管理',
	fit:true,
	fitColumns:true,
	singleSelect:true,
	...
	columns:[[   
         ...
         {field:'repeatNum',title:'周期最低重复次数'},
         ...
         {field:'opt',title:'操作',align:'right',
       		formatter:function(value, row, index) {
   				return '<button onclick="updateBATRule('+index +')" class="easyui-linkbutton">修改</button>';
     		} 
         }
    ]],
    toolbar: [{
			...
			if(node){
				$.messager.confirm('确认', '是否要删除规则"'+node.ruleName+'"?', function(result){
					if (result){
						$.ajax({
							type:'POST',
							url:'${ctx}/base/creditRule/bat/delete', 
							data:{
								action:node.action,
								ruleId:node.ruleId
							},
...
function addBATRule(){
	$('#editCreditRule').dialog({
		...
		buttons:[{
			text:'保存',
			handler:function(){
				submitData('${ctx}/base/creditRule/bat/add');
			}
		},{
			text:'取消',
			handler:function(){
...
function updateBATRule(rowId){
	$('#editCreditRule').dialog({
		...
		buttons:[{
			text:'保存',
			handler:function(){
				submitData('${ctx}/base/creditRule/bat/update');
			}
		},{
			text:'取消',
			handler:function(){
