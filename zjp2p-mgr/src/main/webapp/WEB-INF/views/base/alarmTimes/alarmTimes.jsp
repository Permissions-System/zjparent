...
<div class="easyui-layout" data-options="fit:true,selected:true">
	...
	<div data-options="region:'center',title:'日累计报警次数列表'" style="padding:5px;background:#eee;">
		<table id="table" data-options="fit:true,rownumbers:true,singleSelect:true,pagination:true" style="width:100%"> 
		</table>     
	</div>
</div>
...
<div id="errorLogMonitorDlg" class="easyui-dialog" style="width:430px;height:250px;padding:10px 0px 10px 30px"
    ...
    <form id="errorLogMonitorForm" method="post">
        ...
        <tbody>
            ...
            <tr class="itemMagin">
                <td><label for="idAlarmTimes">日累计报警次数:</label></td>
                <td><input id="idAlarmTimes" name="alarmTimes" class="easyui-validatebox" style="width:155px;"></td>
            </tr>
        </tbody>
        </table>
    </form>
		        ...
		        <table style="width:100%;magrin-top:1px;magrin-left:5px;magrin-right:5px;magrin-bottom:1px;padding:10px 0px 10px 30px">
			        <tbody>
			            <tr class="fitem">
			                ...
			                <td>
			                    <select id="erProductCode"  name="productCode" class="easyui-combobox" style="width:120px;" data-option="editable:false">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    // 周期阀值 限制只数字输入
	    $("#idAlarmTimes").keyup( function() {
	    	validateNum_onKeyUp($("#idAlarmTimes"));
	    });
	...
	function saveErrorLogInfo(){
	    ...
	    if(action=='insert'){
	        ...
	        jQuery.ajax({
	            type : "POST",
	            async: false,
	            url : "${ctx}/base/alarmTimes/insert",
	            data : {
	            	// 错误码
	            	errorCode: $("#idErrorCode").val(),
	            	// 日累计报警次数
	            	alarmTimes: $("#idAlarmTimes").val()	            	
	            },
	        ...
	        jQuery.ajax({
	            type : "POST",
	            async: false,
	            url : "${ctx}/base/alarmTimes/update",
	            data : {
	            	// 错误码
	                errorCode: $("#idErrorCode").val(),
	             	// 日累计报警次数
	    ...
	    $.messager.confirm('确认','确定删除当前选择的配置信息?',function(r){
	        if (r){
	            jQuery.ajax({
	                ...
	                type : "POST",
	                async: false,
	                url : "${ctx}/base/alarmTimes/delete",
	                data : {
	...
	function checkValiErrorMont(){	
	    ...
	    // 日报警累计值
	    val = $("#idAlarmTimes").val();
	    if(isNull(val)){
	        showSlideMessage("请输入日报警累计值！");
	        return false;
	        ct++;
	    }
	...
	function initErrorLogGrid(){
	    $("#table").datagrid({
	        ...
	        singleSelect:true,
	        fit:true,
	        url:'${ctx}/base/alarmTimes/getAlarmTimesList',
	        onDblClickRow:function(rowIndex, rowData){
	        	editErrorLogInfo();
	        },
	        columns:[[
	                  {field:'errorCode',title:'错误码',align:'center',halign:'center'},     
					  {field:'alarmTimes',title:'日累计报警次数',align:'center',halign:'center'}
	              ]],
	        toolbar:[{
		            text:'新增',
		            iconCls:'icon-add',
	...
	function addErrorLogInfo(){
	    ...
	    jQuery.ajax({
	        type : "POST",
	        async: false,
	        url : "${ctx}/base/alarmTimes/getCountCodeInfo",
	        data : {
	        	errorCode:errorCode
	        },
	        success : function(data) {
