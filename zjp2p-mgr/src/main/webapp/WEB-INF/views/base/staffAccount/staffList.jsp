...
<div class="easyui-layout" data-options="fit:true,selected:true">
	...
	<!-- begin 组织机构  -->
	<div data-options="region:'west',split:true,title:'组织机构'" style="width: 200px">
		<ul id="staffOrg" data-options="url:'${ctx}/base/org/orgTree?timeid='+Math.random(),method:'post',checkbox:true,cascadeCheck:false"></ul><!-- url后增加随机数，防止IE缓存造成的不刷新 -->
	</div>
	<!-- end 组织机构 -->
	...
	<div data-options="region:'center',border:false,fit:true" title="人员列表" >		
		...
		<div id="staffTool">
			...
			<select id="staffTypeTwo" name="staffTypeTwo" panelHeight="auto" class="easyui-combobox" style="width:200px" placeholder="员工类型">
		...
		<table>
			<tr style="display: none">
	   		...
	   		<tr>
	   			...
	   			<td>
		   			<select id="staffStatus" name="staffStatus" panelHeight="auto" class="easyui-combobox" panelHeight="auto" data-options="required:true,editable:false">
			   			...
			   			<%-- <c:forEach items="${selects.ENABLED}" var="type">
			   				<option value="${type.configCode}">${type.configName}</option>
			   			</c:forEach> --%>
			   		</select>
		   		</td>
	   		...
	   		<tr>
	   			...
	   			<!-- <input class="easyui-validatebox" type="text" id="channelCode" name="channelCode" data-options="required:true,validType:['id','idExist'] ,validType:['channelCode','length[1,20]']" > -->
	   			</td>
	   		</tr>
	   		</table>
		</form>
	...
	columns:[[        
        {field:'orgCode',title:'所属机构',
        	formatter : function(value, row, index) {
        		var node = $('#staffOrg').tree('find', value);
        		return node.text;
...
$("#staffOrg").tree({
	onCheck:function(node){
		var nodes = $('#staffOrg').tree('getChecked');
		var codes = "";
		...
		//选中组织结构中的菜单时，触发 
		$('#staffTable').datagrid('options').url = '${ctx}/base/staff/staffByOrgList';
...
/* $('#search').click(function(){
	$('#staffTable').datagrid('options').url = '${ctx}/base/staff/search';
	$('#staffTable').datagrid('load',{
...
function doSearchTwo(){
	var condition,ChannelCode,staffType;
	condition = $("#condition").val();
	ChannelCode = $("#ChannelCodeTwo").val();
	staffType = $('#staffTypeTwo').combobox('getValue');
	$('#staffTable').datagrid('options').url = '${ctx}/base/staff/search';
	$('#staffTable').datagrid('load',{
		condition:condition,
...
var staffListOrgTreeEventSwitcher = true;// 组织机构树事件开关
// 设置员工角色
