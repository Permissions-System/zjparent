...
#tinybox {
	...
	display:none; 
	padding:10px; 
	background:#ffffff url(${ctx}/static/images/preload.gif) no-repeat 50% 50%; 
...
<body>
	<table style="width:100%;border:0px;align:center;cellpadding:0;cellspacing:0">
		...
		<tr>
			<td>
				<table id="personInfoTable" style="width:95%;border:0px;cellpadding:0;cellspacing:0">
					<tr>
						<td class="trwidth">
	...
	<hr style="width:100%;align:center;size:0;">
	<table style="width:100%;border:0;align:center;cellpadding:0;cellspacing:0;">
		<tr>
			<td>
			<table id="empAuditTable" style="width:95%;border:0;cellpadding:0;cellspacing:0;">
				<tr>
			...
			if(result == 1) {
				if($.trim(description) == "") {
					...
					$.messager.confirm('确认信息', '确定提交审核意见?', function(r){
						if(r){
							$.ajax({   
								url: '${ctx}/base/employee/employeeAudit', 
								type : 'POST',
								dataType: 'json',
								async: false,
								cache: false,
								...
								success: function(data, status) {
									$.messager.alert('消息', '驳回成功!','info', function(){
										self.parent.reloadTabGrid('法人代表待审核列表','#itEmployeeInfo');
	...
	function viewPersonIdentAndValidate(fileNo){
		...
		if(identStatus==1) {
			...
			if(!isP1FilePathError) {
				if($.trim(fileNo)!="") {
						self.parent.openTab('联网核查：'+name,'${ctx}/base/employee/onlineValidate/'+ userNo);
					} else{
					...
					$.messager.confirm('温馨提示', '该客户提交的身份证正面文件路径存在异常，无法成功打开文件图片进行身份校验，请予以驳回！', function(r){
						if(r){
							$.ajax({
								...
								async : false,
								cache : false,
								url : '${ctx}/base/employee/identFail',
								data : {
									userNo : userNo,
									identOpinion : identOpinion
								},
								success: function(data, status) {
									$.messager.alert('消息', '该法人身份证正面文件驳回成功!','info', function(){
