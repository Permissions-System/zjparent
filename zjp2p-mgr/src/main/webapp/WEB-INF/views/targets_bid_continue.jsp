...
$(document).ready(function(){
	...
	function setVal(){
		...
		if($("#bidnum").val() == "" && $("#amount").val()!= ""){
			var mun = parseInt($("#amount").val())/parseInt($("#mininvestamount").val());
			$("#bidnum").val(mun);
		}
	...
	$("#publish").bind("click",function(){
		...
		if(submitValidate()){
			butDisabled($('#publish'));
			$("#myform").attr("action","saveAndPublishBid.do").submit();
			}
	});
	...
	function submitValidate(){
		...
		if($("input[name=bidtypeid]:checked").val() != '4'){
			...
			$("#yearservicefee").require(rem).match(parnt3, msg);
			$("#riskmgrfee").require(rem).match(parnt3, msg);
			$("#pubfee").require(rem).match(parnt3, msg);
			$("#unpaidfee").require(rem).match(parnt3, msg);
			$("#infofee").require(rem).match(parnt3, msg);
			$("#overduefee").require(rem).match(parnt3, msg);
...
//	 	        alertMsg.error("调用方法发生异常:"+JSON.stringify(err)+"err1"+ JSON.stringify(err1)+"err2:"+JSON.stringify(err2));  
	...
	console.info($("#bidimg").val().indexOf("/"));
	if($("#bidimg").val().indexOf("/") == 0){
		$("#ischecked").html("").html("已上传");
		$('#check').attr("checked", true);
		$("#showimg").attr("src","${ctx}"+$("#bidimg").val());
	}
...
<div class="backcontent">
  ...
  <form class="am-form" id="myform" action="" name="bidForm" method="post">
    ...
    <div class="am-form-group">
      ...
      <c:if test="${bid.bidtypeid == b.key}">
       <label class="am-radio-inline" style="margin-top:-6px;">
        <input type="radio"  value="${b.key}" name="bidtypeid" style="margin-top:8px;" checked="checked"> ${b.value}
      </label>
      <label for="bidtypeid"></label>
      </c:if>
      <c:if test="${bid.bidtypeid != b.key}">
       <label class="am-radio-inline" style="margin-top:-6px;">
        <input type="radio"  value="${b.key}" name="bidtypeid" style="margin-top:8px;"> ${b.value}
      </label>
      <label for="bidtypeid"></label>
      </c:if>
      </c:forEach>
...
<div class="am-form-group am-input-group-sm">
            <label for="user-name" class="am-u-sm-2 am-form-label" style="margin-top:-5px;">产品编号：</label>
            <div class="am-u-sm-10">
            ${bid.bidcode}
            </div>
            <input type="hidden" name="bidcode" value="${bid.bidcode}" >
          </div>
          <div class="am-divider"><hr>
</div>
           ...
           <div class="am-form-group am-input-group-sm">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-input-sm" id="bidclassname" name="bidclassname" placeholder="产品名称" style="float:left; width: 55%;" value="${bid.bidclassname}"><small style="color: black;" maxlength="18">9个字以内</small><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
          <div class="am-form-group am-input-group-sm">
						...
						<div class="am-u-sm-10">
							<select id="useProject" name="projectcode" style="width: 55%;" class="am-input-sm">
								...
								<c:if test="${b.projectcode == bid.projectcode}">
									<option value="${b.projectcode}" account="${b.account}" bidamount="${b.bidamount }" instcode="${b.instcode}" instname="${b.instname }" selected="selected">${b.projectname}</option>
								</c:if>
								<c:if test="${b.projectcode != bid.projectcode}">
					...
					<div class="am-form-group am-input-group-sm">
						...
						<div class="am-u-sm-10">
							...
							<input type="hidden" name="bidimg" id="bidimg" value="${bid.bidimg}" />
						</div>
					</div>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <textarea class="" rows="5" id="bidclasssubject" name="bidclasssubject" maxlength="64">${bid.bidclasssubject}</textarea>
...
<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<textarea class="" rows="5" id="projectfeature"
								name="projectfeature" maxlength="300">${bid.projectfeature}</textarea>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-input-sm" id="investperiod" name="investperiod" style="float:left; width: 25%;" ><span style="float:left; height:40px; line-height:40px; margin-left:10px;">个月</span><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-input-sm" id="raiseperiod" name="raiseperiod" style="float:left; width: 150px;" ><span style="float:left; height:40px; line-height:40px; margin-left:10px;">天</span><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-form-field am-radius" data-am-datepicker="{format: 'yyyy-mm'}" id="exptinterestdate" name="exptinterestdate" style="float:left; width: 250px;" ><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-form-field am-radius" data-am-datepicker="{format: 'yyyy-mm'}" id="exptexpiredate" name="exptexpiredate" style="float:left; width: 250px;" ><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <select id="repaymentway" name="repaymentway" style="width:55%;" class="am-input-sm">
        ...
        <c:forEach items="${repaymentway}" var="b">
          ...
          <div class="am-form-group">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-form-field am-radius" data-am-datepicker="{format: 'yyyy-mm'}" id="exptlastacctdate" name="exptlastacctdate" style="float:left; width: 250px;" ><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
          </div>
  ...
  <div class="am-modal-dialog" style="height:500px;">
     <form  class="am-form am-form-horizontal" style="width:90%; margin-top:10px; margin-left:auto; margin-right:auto">
        ...
        <div class="am-form" style="font-size:13px;">
          ...
          <div class="am-u-sm-9" style="text-align:left">
            <input type="text" id="itouzifee" name="itouzifee" value="<fmt:formatNumber value="${bid.annualizedrate}" groupingUsed="false" type="number" maxFractionDigits="2"/>" disabled="disabled" class="am-input-sm"  autocomplete="off" style="width: 150px; height:30px;float:left;  padding-bottom:0px;margin-top:15px;">
            <span style="float: left; height: 40px; line-height: 40px; margin-left: 10px;margin-top:10px;">%</span> 
            </div>
        </div>
          ...
          <div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<input type="text" class="am-input-sm" id="pubfee" disabled="disabled"
								name="pubfee" style="float: left; width: 150px;" value="<fmt:formatNumber value="${bid.pubfee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"><span
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<input type="text" class="am-input-sm" id="yearservicefee" disabled="disabled"
								name="yearservicefee" style="float: left; width: 150px;" value="<fmt:formatNumber value="${bid.yearservicefee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"><span
								style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">%</span><span
								style="float: left; color: #F00; height: 40px; line-height: 40px; margin-left: 10px; font-size: 20px;">
								*</span>（按月收取，进入流动活动金）
						</div>
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10"><span style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">借款人费率：</span>
							<input type="text" class="am-input-sm" id="borrowerinterest"  name="borrowerinterest" disabled="disabled" style="float: left; width: 150px;" value="${bid.borrowerinterest}">
							<span style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">% - 投资人费率：</span> 
							<span style="float: left; height: 40px; line-height: 40px; margin-left: 10px;" id="touzifee"><fmt:formatNumber value="${bid.annualizedrate}" groupingUsed="false" type="number" maxFractionDigits="2"/></span>
							<span
								style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">%</span><span
								style="float: left; color: #F00; height: 40px; line-height: 40px; margin-left: 10px; font-size: 20px;">
								*</span>（按月收取，进入流动活动金）
								<input type="text" class="am-input-sm" id="infofee" name="infofee"  disabled="disabled" style="float: left; width: 150px;" value="${bid.infofee}">
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<input type="text" class="am-input-sm" id="overduefee" disabled="disabled"
								name="overduefee" style="float: left; width: 150px;" value="<fmt:formatNumber value="${bid.overduefee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"><span
								style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">%</span><span
								style="float: left; color: #F00; height: 40px; line-height: 40px; margin-left: 10px; font-size: 20px;">
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<input type="text" class="am-input-sm" id="unpaidfee" disabled="disabled"
								name="unpaidfee" style="float: left; width: 150px;" value="<fmt:formatNumber value="${bid.unpaidfee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"><span
								style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">%</span><span
								style="float: left; color: #F00; height: 40px; line-height: 40px; margin-left: 10px; font-size: 20px;">
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<input type="text" class="am-input-sm" id="riskmgrfee" disabled="disabled"
								name="riskmgrfee" style="float: left; width: 150px;" value="<fmt:formatNumber value="${bid.riskmgrfee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"><span
								style="float: left; height: 40px; line-height: 40px; margin-left: 10px;">%</span><span
								style="float: left; color: #F00; height: 40px; line-height: 40px; margin-left: 10px; font-size: 20px;">
        ...
        <div class="am-form-group am-input-group-sm">
            ...
            <div class="am-u-sm-10">
              <input type="text" class="am-input-sm" id="bidnum" name="bidnum" placeholder="不填写默认为剩余总份数" style="float:left; width: 40%;" value="${bid.bidnum}"><span style="float:left; height:40px; line-height:40px; margin-left:10px;">份</span><span style="float:left; color:#F00; height:40px; line-height:40px; margin-left:10px; font-size:20px;"> *</span>
            </div>
        </div>
        ...
        <div class="am-form-group">
						<label class="am-form-label" style="margin-left: 50px;"> <input
							type="checkbox" id="ifredeem" name="ifredeem" value="1" <c:if test="${bid.ifredeem == '1' }"> checked="checked"</c:if>>
					...
					<div class="am-form-group" id="ifredeem2" style="display: none;">
						<label class="am-u-sm-3" style="margin-left: 70px;"> 
							按投资总额比例计算
						</label> <input type="text" class="am-input-sm" id="redeemfee" name="redeemfee" value="<fmt:formatNumber value="${bid.redeemfee}" groupingUsed="false" type="number" maxFractionDigits="2"/>"
					...
					<div class="am-form-group">
						...
						<div class="am-u-sm-10">
							<textarea class="" rows="5" id="ensuretype"
								name="ensuretype" maxlength="1000">${bid.ensuretype}</textarea><small>500字以内<span style="color: #F00;"> *</span></small>
...
<div class="am-form-group">
      ...
      <label class="am-form-label" style="margin-left:50px;">前台发布时间：</label>
        ...
        <div class="am-form-group">
      <label class="am-u-sm-3 " style=" margin-left:70px;">
        <input type="radio" name="doc-radio-1" value="1" checked > 直接发布   
      </label>      
    </div>
    ...
    <div class="am-divider"><hr>
				...
				<div class="am-form-group">
					<label class="am-form-label">标的属性：</label>
					 <input type="radio" name="ifvirtual" value="0" 
					 <c:if test="${bid.ifvirtual eq 0 }"> checked="checked"</c:if> >真实标
					 <input type="radio" name="ifvirtual" value="1" 
					 <c:if test="${bid.ifvirtual eq 1 }"> checked="checked"</c:if> >模拟标
				</div>
          
      </div>
    <div class="tagconfirm"><button type="button" id="save" class="am-btn am-btn-secondary">保 存</button> <button type="button" id="publish" class="am-btn am-btn-secondary">保存并发标</button> 
