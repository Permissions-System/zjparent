...
<div class="backcontent">
  ...
  <div class="searchpanel">
    <form class="am-form-inline" role="form">
      ...
      <div class="am-form-group" style="margin-top:10px;">
        <label for="doc-ipt-3" class="am-u-sm-6 am-form-label" style="width: 180px;">担保机构编号：</label>
        <div class="am-u-sm-2" style="float:left; margin-left:-45px;"><input class="am-form-field am-input-sm" type="text"  placeholder="请输入担保机构编号：" style="width:150px; height:30px;padding-top:0px; padding-bottom:0px;" value="${map.instcode}" id="instcode" name="instcode"></div>
      </div> 
      <div class="am-form-group" style="margin-top:10px;">
        <label for="doc-ipt-3" class="am-u-sm-6 am-form-label" style="width: 180px;">担保机构名称：</label>
  ...
  <a href="editVouchOrg.do"><button type="button" class="am-btn am-btn-primary" style="margin-left:15px; margin-top:20px; float:left;"><span class=" am-icon-plus-square">　添加担保机构</span></button></a>
  ...
  <div class="tablecontent"><table class="am-table am-table-bordered am-table-striped am-table-hover" style="text-align:center;">
    <thead>
        <tr style="text-align:center;">
            ...
            <th style="text-align:center;">担保机构编号</th>
            <th style="text-align:center;">担保机构名称</th>
            <th style="text-align:center;">加入时间</th>
            <th style="text-align:center;">当前状态</th>
            <th style="text-align:center;">操 作</th>
        </tr>
    </thead>
    <c:forEach items="${page.results}" var="vouchOrg" varStatus="status">
    <tbody>
        <tr>
            <td>${status.count}</td>
            <td>${vouchOrg.instcode}</td>
            <td>${vouchOrg.instname}</td>
            <td><fmt:formatDate value="${vouchOrg.validdate}" type="both"/></td>
            <td><c:forEach items="${orgState}" var="o">
					 <c:if test="${vouchOrg.orgstate==o.key}">${o.value}</c:if>
				</c:forEach></td>
            <td><a href="viewVouchOrg.do?id=${vouchOrg.id}" target="_blank"><button type="button" class="am-btn am-btn-secondary am-btn-sm">查看</button></a>
            	<a href="editVouchOrg.do?id=${vouchOrg.id}"><button type="button" class="am-btn am-btn-secondary am-btn-sm">编辑</button></a>
            	<c:if test="${vouchOrg.orgstate == 0}"><a href="changeOrgstate.do?id=${vouchOrg.id}&orgstate=${vouchOrg.orgstate}" ><button type="button" class="am-btn am-btn-secondary am-btn-sm" onclick="return confirm('您确定要启用该担保机构吗?*该担保机构启用后,可作为发布标的时待关联担保机构的选项出现')">启用</button></a></c:if>
            	<c:if test="${vouchOrg.orgstate == 1}"><a href="changeOrgstate.do?id=${vouchOrg.id}&orgstate=${vouchOrg.orgstate}" ><button type="button" class="am-btn am-btn-secondary am-btn-sm" onclick="return confirm('您确定要禁用该担保机构吗?*该担保机构被禁用后,发布标的时无法关联到该担保机构')">禁用</button></a></c:if>
            	<button type="button" class="am-btn am-btn-secondary am-btn-sm" onclick="deleteVouchOrg(${vouchOrg.id})">删除</button>
            </td>
        </tr>
    </tbody>
    </c:forEach>
...
<form id="getTopost" action="queryVouchOrgList.do" method="post">
...
function deleteVouchOrg(id){
	$.ajax({ 
		dataType: "json",
	    type:'post',
	    url:'deleteVouchOrg.do?id='+id,     
	    success:function(data){
	    	if("1" == data.status){
	    			alert('删除成功');
	        		location.href = "queryVouchOrgList.do";
	        	}
	    	if("0" == data.status){
	    		alert('已关联项目，无法删除');
        		location.href = "queryVouchOrgList.do";
