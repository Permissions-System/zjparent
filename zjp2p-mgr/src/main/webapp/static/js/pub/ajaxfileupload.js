jQuery.extend({
	...
	ajaxFileUpload : function(s) {
		...
		var fileValue = $("#jQueryFormFile" + s.fileElementId).val();
				...
				success : function(data) {
					...
					if (data != "") {
						try {
							...
							$.messager.alert("提示", "对不起，文件服务器故障！请与管理员联系。", "warn");
							return;
						}
					}
					if ("success" == json.status) {
						...
						if (typeof s.callback == 'function') {
							window.setTimeout(function() {
								$(".ui-file-select", $("#" + s.fileElementId)).val(fileValue);
							}, 1000);
