...
*/
  ...
  {
    this.logger = LoggerFactory.getLogger(StaffAccountService.class);
  }
  ...
  public Boolean checkChannelCodeExist(String channelCode)
		    throws BusinessException
		  {
		    Staff st = this.staffDao.selectByChannelCode(channelCode);
		    if (st != null) {
		      return Boolean.valueOf(false);
		    }
  ...
  public Page<Staff> getStaffByOrgListTwo(Map<String, Object> params, int pageNo, int pageSize)
    throws BusinessException
  {
    Page page = new Page();
    page.setPageNo(pageNo);
    page.setPageSize(pageSize);
    params.put("pageObject", page);
    page.setResult(this.staffDao.selectByOrgListTwo(params));
    return page;
  }
  ...
  public Page<Staff> getStaffBySearchTwo(Map<String, Object> params, int pageNo, int pageSize)
		    throws BusinessException
		  {
		    Page page = new Page();
		    page.setPageNo(pageNo);
		    page.setPageSize(pageSize);
		    params.put("pageObject", page);
		    page.setResult(this.staffDao.selectByNameSearchTwo(params));
		    return page;
		  }
  ...
  {
	// 随机生成数组，如：[47, -56, -58, 64, -75, 102, -9, -96]
    byte[] salt = DigestUtils.generateSalt(8);
    // 将数组转换成16位字符串，如：30222b160c7f5bc5
    staff.setSalt(EncodesUtils.encodeHex(salt));
/*    // 将16位字符串转换成数组，如：[47, -56, -58, 64, -75, 102, -9, -96]
    byte[] a = EncodesUtils.decodeHex(EncodesUtils.encodeHex(salt));
    String b = this.encodePwd("123.com", "30222b160c7f5bc5");*/
   ...
   * @return
   */
  public String encodePwd(String pwd,String salt)
  {
	  String result = null;
	  byte[] arr = null;
	  byte[] hashPassword = null;
	  
	  arr = EncodesUtils.decodeHex(salt);
	  hashPassword = DigestUtils.sha1(pwd.getBytes(), arr, 1024);
	  result = EncodesUtils.encodeHex(hashPassword);
    ...
    if ((list != null) && (list.size() > 0))
      for (StaffRoleRel rel : list)
        if (rel == null) {
          ...
          this.logger.error("role is null... 当前员工编号: " + staffNo);
        } else {
    ...
    throws BusinessException
  {
    ...
    List<Role>  listA = this.staffDao.findRoleByOrgCodeForAdminStaff(map);
...
@Override
public Staff selectByChannelCode(String ChannelCode ) throws BusinessException
{
	return this.staffDao.selectByChannelCode(ChannelCode);
}

@Override
public Map<String, String> selectByIdentNoAndPwd(String identNo,String pwd ) 
		throws BusinessException
{
	...
	staff = this.staffDao.selectByIdentNo(identNo);
	
	//0登入成功、1用户名为空、2密码为空、3用户名错误、4密码错误
	if(identNo==null && "".equals(identNo))
	{
		message = "用户名不能为空";
		status = "1";
	}
	...
	{
		salt = staff.getSalt();
		encodePwd = this.encodePwd(pwd, salt);
		System.out.println("----------------------------------------------------------------------------   "+encodePwd);
		
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("identNo", identNo);
		paramMap.put("pwd", encodePwd);
		staff = this.staffDao.selectByIdentNoAndPwd(paramMap);
		if(staff==null)
		{
			message = "密码错误";
			status = "4";
