(function(){
	...
	function createSubject(count){
		...
		if(count < len){
			html += '<h2 style="font-size:14px; color:#565A5C; font-weight:normal">'+array[count][0]+'.'+array[count][1]+'：</h2>';
			...
			html += '<a href="javascript:;" id="NextTopic" data-rel="'+count+'" class="am-btn am-btn-secondary">'+(count == array.length - 1 ? '完成':'下一步')+'</a>';
	...
	function worthResult(ret,len){
		...
		var html = '<li>坚持在百富网投资，'+estTime+'年后您的身价将为<span class="light-org">'+sum+'</span>元，增长<span class="light-org">'+grow+'%</span></li>';
	...
	function worthCharting(worth, step){
		$('.worth-chart').html('<div class="worth-div worth-'+step+'"><h2>预期身价(元)</h2><strong>'+worth+'</strong></div>');
	};
	worthCharting(0, 0);
	createSubject(0);
})();