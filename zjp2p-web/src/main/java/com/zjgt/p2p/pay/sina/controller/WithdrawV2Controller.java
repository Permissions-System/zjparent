...
@Controller
@RequestMapping({"/pay/withdrawV2"})
public class WithdrawV2Controller
{

  @Autowired
  private PayV2Service payService;
  ...
  public String back(HttpServletRequest request) {
    boolean flag = false;
    Map resultMap = null;
    BatchPay2bankResponse payResponse = null;
    try {
      resultMap = WebUtil.getParameterMap(request, false);
      payResponse = (BatchPay2bankResponse)MapToBean.getBeanV2(resultMap, BatchPay2bankResponse.class);
      PayLogUtil.log("withdraw back response :{}", new Object[] { payResponse });
      flag = this.payService.withdrawRespond(payResponse);
      PayLogUtil.log("withdraw back response process result :{} , batchNumber is {}", new Object[] { Boolean.valueOf(flag), payResponse.getBatch_no() });
    } catch (Exception exception) {
