...
@Controller
@RequestMapping({"/pay/ll/authRecharge"})
public class RechargeAuthController
{
  ...
  public String recharge(Model model, Integer orderAmount, HttpServletRequest req)
  {
    ...
    PayLogUtil.log("LianLian authRecharge request, memberId:{},orderAmount:{}", new Object[] { currUser.getId(), orderAmount });
    if ((StringUtils.isEmpty(currUser.getIdcard())) || (StringUtils.isEmpty(card_no))) {
      PayLogUtil.log("LianLian authRecharge request, 未认证,无法认证支付, {} , {} , {}", new Object[] { currUser.getId(), currUser.getIdcard(), card_no });
    ...
    try
    {
      ...
      payRequest = plainPay(req, order, card_no);
      PayLogUtil.log("LianLian authRecharge request :{}", new Object[] { payRequest.toString() });
      String generateForm = generateForm(payRequest);
      PayLogUtil.log("LianLian authRecharge request generatedForm:{}", new Object[] { generateForm });
  ...
  public String generateForm(PaymentInfo order) {
    ...
    String actionUrl = PropertiesUtils.getProperties("pay.ll.authpayRecharge.actionUrl");
  ...
  public String back(HttpServletRequest request) {
    ...
    if (LLPayUtil.isnull(reqStr)) {
      PayLogUtil.log(new StringBuilder().append("LianLian /pay/ll/authRecharge/back.do;reqStr is null ,").append(reqStr).toString());
      retBean.setError();
    ...
    try {
      ...
      PayLogUtil.log("LianLian authRecharge back response :{}", new Object[] { payResponse });
      processResult = this.thirdPayService.fillRespond(payResponse, reqStr);
      PayLogUtil.log("LianLian authRecharge back response process result :{}", new Object[] { Boolean.valueOf(processResult) });
    } catch (Exception exception) {
  ...
  public String success(HttpServletRequest request)
  {
    ...
    String reqStr = null;
    try {
      payResponse = (PaymentNotifyRetBean)MapToBean.getBean(resultMap, PaymentNotifyRetBean.class);
      reqStr = JSON.toJSONString(payResponse);
      PayLogUtil.log("LianLian authRecharge success response :{},reqStr :{}", new Object[] { payResponse, reqStr });
      processResult = this.thirdPayService.fillRespond(payResponse, reqStr);
      PayLogUtil.log("LianLian authRecharge success response process result :{}", new Object[] { Boolean.valueOf(processResult) });
    } catch (Exception exception) {
      StringBuilder builder = new StringBuilder();
      builder.append(new StringBuilder().append("reqStr:").append(reqStr).append("\n").toString());
      if (payResponse != null) {
      ...
      PayLogUtil.log(new StringBuilder().append("LianLian authRecharge 充值同步烦返回处理失败 , ").append(builder.toString()).toString(), exception);
    }
  ...
  private PaymentInfo plainPay(HttpServletRequest req, OrderInfo order, String card_no)
  {
    ShiroMember currUser = ShiroWebUtil.getCurrentUser();
    ...
    paymentInfo.setNotify_url(PropertiesUtils.getProperties("pay.ll.authpayRecharge.notify_url"));
    paymentInfo.setUrl_return(PropertiesUtils.getProperties("pay.ll.authpayRecharge.url_return"));
    ...
    paymentInfo.setId_no(currUser.getIdcard());
    paymentInfo.setCard_no(card_no);
    paymentInfo.setAcct_name(currUser.getRealname());
