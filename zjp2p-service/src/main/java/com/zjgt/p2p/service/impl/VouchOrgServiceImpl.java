...
@Service("vouchOrgService")
public class VouchOrgServiceImpl
  implements VouchOrgService
{
  private static final String PATTERN = "yyyy-MM-dd";

  @Autowired
  private VouchOrgMapper orgMapper;

  @Autowired
  private BidService bidService;

  public VouchOrg findById(String id)
  {
    VouchOrg org = null;
    if (id != null) {
  ...
  public List<VouchOrg> selectAll()
  {
    VouchOrgExample example = new VouchOrgExample();
  ...
  public Page<VouchOrg> selectVouchOrgByPage(Map<String, String> map)
  {
    Page pages = null;

    List vouchOrgList = null;
    VouchOrgExample exam = new VouchOrgExample();
    VouchOrgExample.Criteria crit = exam.createCriteria();
    ...
    int totalCount = this.orgMapper.getVouchOrgCount(exam);
    int pageNow = 0;
    ...
    vouchOrgList = this.orgMapper.selectByExampleLimit(exam);
    pages.getPager(pageNow, pages.getTotalPageCount());
    pages.setResults(vouchOrgList);
    return pages;
  }

  public int addVouchOrg(VouchOrg vouchOrg)
  {
    return this.orgMapper.insertSelective(vouchOrg);
  }

  public int updateVouchOrg(VouchOrg vouchOrg)
  {
  ...
  public VouchOrg findBybid(String id)
  {
    ...
    VouchOrgExample example = new VouchOrgExample();
    example.createCriteria().andInstcodeEqualTo(bid.getInstcode());
    List list = this.orgMapper.selectByExample(example);
    if ((list != null) && (!(list.isEmpty()))) {
      return ((VouchOrg)list.get(0));
    }
    return null;
  }

  public VouchOrg findByInstcode(String instcode)
  {
    VouchOrgExample example = new VouchOrgExample();
