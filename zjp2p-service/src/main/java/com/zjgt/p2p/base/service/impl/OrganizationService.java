  ...
  implements IOrganizationService
{
  ...
  public Organization getOrganizationByUserId(String uid)
    throws BusinessException
  {
    ...
    throws BusinessException
  {
    ...
    List<Organization> childrenOrg = this.organizationDao.selectAllChildenOrgs(orgTreePath + "/");
    childrenOrg.add(org);
    
    Organization child;

    for (Iterator i$ = childrenOrg.iterator(); i$.hasNext(); ) { 
    	child = (Organization)i$.next();
    ...
    try
    {
      for (Iterator j$ = childrenOrg.iterator(); j$.hasNext(); ) { child = (Organization)j$.next();
